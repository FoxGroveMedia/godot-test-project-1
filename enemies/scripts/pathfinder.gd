...
# Set the best_path vector by checking for the best direction and considering obstacles.
func set_path() -> void:
	...
	# If there are no obstacles, recommend path in the direction of the player.
	if obstacles.max() == 0:
	...
	# Set the best path with the Vector2 that corresponds with the outcome
